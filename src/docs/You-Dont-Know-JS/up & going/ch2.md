...
**注意：** 更多关于JS的值和它们的类型的信息，参见本系列的 *类型与语法* 的前两章。
...
**注意：** 关于JS原生类型和“封箱”的更多信息，参见本系列的 *类型与语法* 的第三章。要更好地理解对象原型，参见本系列的 *this与对象原型* 的第五章。
...
强制转换并不邪恶，它也不一定是令人吃惊的。事实上，你使用类型强制转换构建的绝大部分情况是十分合理和可理解的，而且它甚至可以用来 *增强* 你代码的可读性。但我们不会在这个话题上过度深入 —— 本系列的 *类型与语法* 的第四章将会进行全面讲解。
...
**注意：** 更多关于`==`等价性比较规则的信息，参见ES5语言规范（11.9.3部分），和本系列的 *类型与语法* 的第四章；更多关于值和引用的信息，参见它的第二章。
...
**注意：** 关于不等价比较规则的更多信息，参见ES5语言规范的11.8.5部分，和本系列的 *类型与语法* 第四章。
...
**注意：** 更多关于保留字的信息，参见本系列的 *类型与语法* 的附录A。
...
**注意：** 关于测试条件和`switch`与`? :`的其他模式的更多信息，参见本系列的 *类型与语法*。
...
**注意：** 关于strict模式的更多信息，参见本系列的 *类型与语法* 的第五章。
...
围绕在函数表达式`(function IIFE(){ .. })`外部的`( .. )`只是一个微妙的JS语法，我们需要它来防止函数表达式被看作一个普通的函数声明。
