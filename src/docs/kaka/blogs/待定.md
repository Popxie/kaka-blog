 ...
 * @Description:
 * @Author: xiehuaqiang
 * @FilePath: /kaka-blog/src/docs/kaka/blogs/待定.md
 * @Date: 2021-08-25 14:02:07
 * @LastEditTime: 2021-08-25 19:22:54
-->
...
如果你对默认的 `eslint` 规则不太满意那就直接修改规则，根据自己的项目自定义化 `eslint` 规则~~,这里我使用了是其中一种文件格式`.eslintrc.yaml`部分内容如下：
...
- 手动生成
  ...
  - 看看结果
    ...
    总之不管你是 `手动` 生成还是借助 `cz-cli` `自动` 生成，目的只有一个！就是生成特定格式的 `commit` 记录，为后面生成 `CHANGELOG.md` 做下铺垫！
